...
class PiCO(nn.Module):
    ...
    def __init__(self, args, base_encoder):
        ...
        self.encoder_q = base_encoder(num_class=args.num_class, domain_class=args.domain_class, feat_dim=args.low_dim,
                                      pretrained=pretrained)  # name=args.arch,
        # momentum encoder
        self.encoder_k = base_encoder(num_class=args.num_class, domain_class=args.domain_class, feat_dim=args.low_dim,
                                      pretrained=pretrained)  # name=args.arch,
        ...
        self.register_buffer("prototypes", torch.zeros(args.num_class, args.low_dim))  # (3, 128)
    ...
    def forward(self, img_q, im_k=None, partial_Y=None, args=None, eval_only=False):
        ...
        logits_prot = torch.mm(q, prototypes.t())  # (256,128) (128,3)
        ...
        with torch.no_grad():  # no gradient 
            ...
            im_k, predicetd_scores, partial_Y, score_domain, idx_unshuffle = self._batch_shuffle_ddp(im_k,
                                                                                                     predicetd_scores,
                                                                                                     partial_Y,
                                                                                                     score_domain)
            _, k, k_d = self.encoder_k(im_k)
            # undo shuffle
            k, predicetd_scores, partial_Y, score_domain = self._batch_unshuffle_ddp(k, predicetd_scores, partial_Y,
                                                                                     score_domain, idx_unshuffle)
