...
# from utils.seed import get_SEED_domain
from utils.seed_data import get_SEED_domain
...
parser.add_argument('--target-domain',default=0,type=int)
...
parser.add_argument('--not-edb',action="store_true",
                    help='if not use edb')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, loss_fn, loss_cont_fn,loss_domain_fn,vanilla_fn, optimizer, epoch, args, logger, start_upd_prot)
...
def train(train_loader, model, loss_fn, loss_cont_fn,loss_domain_fn, vanilla_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    for i, (images_w, images_s, labels, true_labels, index,domain_label) in enumerate(train_loader):
        ...
        if args.not_edb:
            vanilla_loss = vanilla_fn(cls_out,Y_true)
        ...
        loss_domain = loss_domain_fn(domain_out,domain_label.long())
...
# from resnet import *
#
# sys.path.append('/workspace/PiCOO')
# from utils.utils_algo import *
# from utils.utils_loss import partial_loss, SupConLoss
...
# # from utils.VLCS import get_dataset
# # from pathos.multiprocessing import ProcessingPool as Pool
...
#                     choices=['PACS', 'VLCS'],
#                     help='dataset name (cifar10)')
# parser.add_argument('--target-domain', default='P', type=str)
# parser.add_argument('--exp-dir', default='experiment/PiCO-PACS', type=str,
#                     help='experiment directory for saving checkpoints and logs')
# parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18', choices=['resnet18'],
#                     help='network architecture (only resnet18 used in PiCO)')
# parser.add_argument('-j', '--workers', default=8, type=int,
...
# parser.add_argument('--partial_rate', default=0.25, type=float,
#                     help='ambiguity level (q)')
# parser.add_argument('--hierarchical', action='store_true',
#                     help='for CIFAR-100 fine-grained training')
#
...
#             args.batch_size = int(args.batch_size / ngpus_per_node)
#             args.workers = int((args.workers + ngpus_per_node - 1) / ngpus_per_node)
#             print('!!!!!!!!!!!', args.batch_size, args.workers)
#             model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu])
#         else:
#             model.cuda()
#             # DistributedDataParallel will divide and allocate batch_size to all
...
#     if args.dataset == 'PACS':
#         target_domain = args.target_domain
#         train_loader, train_givenY, train_sampler, test_loader = load_PACS(target_domain,
...
#                                                                            batch_size=args.batch_size)
#     elif args.dataset == 'VLCS':
#         VLCSdataset, _in_splits, _out_splits = get_dataset([0], args, hparams)
#         target_domain = 'S'
#         train_loader, train_givenY, train_sampler, test_loader = load_VLCS(target_domain,
#                                                                            partial_rate=args.partial_rate,
#                                                                            batch_size=args.batch_size)
#         # target_domain= args.target_domain
#         # train_loader, train_givenY, train_sampler, test_loader = load_VLCS(target_domain,partial_rate=args.partial_rate, batch_size=args.batch_size)
#     else:
#         raise NotImplementedError("You have chosen an unsupported dataset. Please check and try again.")
#     # this train loader is the partial label training loader
#
...
#         # torch.Size([32, 3, 224, 224]) torch.Size([32, 3, 224, 224]) torch.Size([32, 7]) torch.Size([32]) torch.Size([32]) torch.Size([32])
#         data_time.update(time.time() - end)
#         X_w, X_s, Y, index = images_w.cuda(), images_s.cuda(), labels.cuda(), index.cuda()
#         Y_true = true_labels.long().cuda()
#         # for showing training accuracy and will not be used when training
